...
TRUNCATED WINDOW VERSION - Fixes particle degeneracy problem.

Key insight: Long sequences (T=2000) without resampling cause particle degeneracy.
The likelihood surface becomes chaotic, making finite-diff gradients garbage.

Solution: Optimize on SHORT windows (50 steps) where particles stay healthy.
This is effectively SGD with finite-diff gradient estimates.
...
- Result: ρ, σ_z, and μ all converge
"""
...
class FiniteDiffOptimizer:
    ...
    def compute_gradient(self, observations: torch.Tensor, seed: int = None,
        ...
        if init_particles is not None:
            # init_particles is [particles] -> expand to [6, particles]
            init_particles = init_particles.unsqueeze(0).expand(6, -1).clone()
...
def train_finite_diff_truncated(
    ...
    Key insight: Long sequences cause particle degeneracy without resampling.
    Solution: Optimize on short windows where particles stay healthy.
    ...
    if verbose:
        print(f"\n{'='*70}")
        print("SVPF FINITE DIFFERENCES - TRUNCATED WINDOW TRAINING")
        print(f"{'='*70}")
        ...
        print(f"True:       ρ={true_params.rho:.3f}, σ_z={true_params.sigma_z:.3f}, μ={true_params.mu:.2f}")
        print(f"Init:       ρ={init_rho:.3f}, σ_z={init_sigma:.3f}, μ={init_mu:.2f}")
        print(f"{'─'*70}")
    ...
    for epoch in range(n_epochs):
        epoch_ll = 0.0
        epoch_grads = {'rho': 0.0, 'sigma': 0.0, 'mu': 0.0}
        ...
        history['grad_sigma'].append(epoch_grads['sigma'])
        ...
        if verbose and (epoch % 20 == 0 or epoch == n_epochs - 1):
            ...
            print(f"Epoch {epoch:4d} ({elapsed:5.1f}s): LL={epoch_ll:8.2f}, "
                  ...
                  f"σ_z={sigma_z:.4f} ({true_params.sigma_z:.3f}), "
                  f"μ={mu:.2f} ({true_params.mu:.1f})")
    
    if verbose:
        print(f"{'─'*70}")
        rho, sigma_z, mu = optimizer.params
        print(f"Final:      ρ={rho:.4f}, σ_z={sigma_z:.4f}, μ={mu:.2f}")
        
        # Compute errors
        print(f"\nErrors:")
        print(f"  |ρ - ρ*|   = {abs(rho - true_params.rho):.4f}")
        print(f"  |σ - σ*|   = {abs(sigma_z - true_params.sigma_z):.4f}")
