"""
SVPF v5 - EKF Gap Bias Correction
===================================

Same as v4 but replaces importance weight correction with EKF gap:
  bias_signal = guide_mean - h_mean_uniform

No circular dependency: guide_mean comes from a standalone Kalman filter
on observations, h_mean_uniform from the particle cloud. They're independent.

Four modes:
  1. base:         Standard SVPF (equal-weight outputs)
  2. ekf_gap:      + Use EKF gap to correct output h_mean only
  3. ekf_feedback: + Feed EKF gap back into guide offset for next step
  4. iw:           + Importance weights (from v4, for comparison)
"""
...
class SVPFConfig:
    ...
    # --- Correction mode (set by harness) ---
    correction_mode: str = "none"  # "none", "ekf_gap", "ekf_feedback", "iw"

    # EKF gap params
    ekf_gap_ema_alpha: float = 0.15
    ekf_gap_output_blend: float = 0.3   # how much to trust EKF vs particles for output
    ekf_feedback_gain: float = 0.5      # how aggressively to feed back

    # IW params (for comparison)
...
class SVPFTorch:
    ...
    def initialize(self, seed: int = 123):
        ...
        # EKF gap state
        self.ekf_gap_ema = 0.0
        self.guide_bias_offset = 0.0

        # IW state (for comparison mode)
        self.iw_bias_ema = 0.0

    # -------------------------------------------------------------------------
    # EKF Guide (CPU scalars)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Guide: variance-preserving shift
    # -------------------------------------------------------------------------
    @torch.no_grad()
    ...
    # -------------------------------------------------------------------------
    # Stein step (Cauchy kernel + RMSProp)
    # -------------------------------------------------------------------------
    @torch.no_grad()
    ...
    # -------------------------------------------------------------------------
    # IW computation (for comparison - same as v4)
    # -------------------------------------------------------------------------
    @torch.no_grad()
    def _compute_importance_weights(self, y_t: float) -> torch.Tensor:
        ...
        h_safe = torch.clamp(h, -10.0, 10.0)
        vol = torch.exp(h_safe)
        A = (y_t * y_t) / (vol * nu + 1e-8)
        log_lik = (self.student_t_const - 0.5 * h
                   - 0.5 * (nu + 1.0) * torch.log1p(A.clamp(min=0.0)))

        mu_k = cfg.mu + cfg.rho * (self.h_prev - cfg.mu)
        diff = h.unsqueeze(1) - mu_k.unsqueeze(0)
        if cfg.use_student_t_state:
            nu_s = cfg.nu_state
        ...
        diff_q = h.unsqueeze(0) - h.unsqueeze(1)
        log_kde = torch.clamp(-0.5 * (diff_q / bw) ** 2, min=-50.0, max=0.0)
        ...
        log_q = (lk_max.squeeze(1) + torch.log(kde_sum.clamp(min=1e-30))
                 - math.log(max(n - 1, 1)) - math.log(bw) - 0.5 * math.log(2.0 * math.pi))

        log_w = torch.clamp(log_lik + log_prior - log_q, -20.0, 20.0)
    ...
    def step(self, y_t: float, y_prev: float = 0.0) -> dict:
        ...
        # 1. EKF guide (independent of particles)
        ...
        # 8. Compute EKF gap (ALWAYS — it's free)
        ekf_gap = self.guide_mean - h_mean_uniform

        # 9. Apply correction based on mode
        result = {
        ...
        if mode == "ekf_gap":
            # Output correction: blend particle mean toward EKF mean
            # Don't trust EKF fully — it's a linear approx
            # Use EMA of the gap as the correction signal
            alpha = cfg.ekf_gap_ema_alpha
            self.ekf_gap_ema = (1.0 - alpha) * self.ekf_gap_ema + alpha * ekf_gap
            
            blend = cfg.ekf_gap_output_blend
            h_mean_corrected = h_mean_uniform + blend * self.ekf_gap_ema
            vol_corrected = math.exp(h_mean_corrected / 2.0)
            
            result["h_mean"] = h_mean_corrected
            result["vol"] = vol_corrected
            result["bias_estimate"] = self.ekf_gap_ema

        elif mode == "ekf_feedback":
            # Same output correction as ekf_gap
            alpha = cfg.ekf_gap_ema_alpha
            self.ekf_gap_ema = (1.0 - alpha) * self.ekf_gap_ema + alpha * ekf_gap
            ...
            # PLUS: feed back into guide offset for next step
            self.guide_bias_offset = cfg.ekf_feedback_gain * self.ekf_gap_ema
            ...
            result["ess"] = ess
            result["bias_estimate"] = h_mean_weighted - h_mean_uniform
            result["var_ratio"] = var_weighted / max(var_uniform, 1e-8)
...
def run_experiment(T=1000, n_particles=512, dgp_cfg=None, filter_cfg=None,
    ...
    # Map mode to correction_mode
    filter_cfg.correction_mode = {
        "base": "none", "ekf_gap": "ekf_gap",
        "ekf_feedback": "ekf_feedback", "iw": "iw"
    }[mode]
    ...
    return {
        ...
        "mean_ekf_gap": float(np.mean(gap_arr)),
        "std_ekf_gap": float(np.std(gap_arr)),
        "time_sec": elapsed,
    }


def compare_modes(T=1000, n_particles=512, dgp_cfg=None, seed_dgp=42, n_runs=5):
    modes = ["base", "ekf_gap", "ekf_feedback", "iw"]

    print("=" * 105)
    print(f"SVPF Bias Correction Comparison  |  T={T}, N={n_particles}, runs={n_runs}")
    ...
    for run in range(n_runs):
        ...
        eg = results_all["ekf_gap"][-1]
        ef = results_all["ekf_feedback"][-1]
        iw = results_all["iw"][-1]
        print(f"  Run {run+1}/{n_runs}: "
              f"base bias={b['mean_bias_h']:+.3f} | "
              f"ekf_gap bias={eg['mean_bias_h']:+.3f} | "
              f"ekf_fb bias={ef['mean_bias_h']:+.3f} | "
              f"iw bias={iw['mean_bias_h']:+.3f} | "
              f"gap={b['mean_ekf_gap']:+.3f} | "
              f"{b['time_sec']:.1f}s")

    print(f"\n{'Mode':<15} {'RMSE_h':>8} {'Bias_h':>8} {'RMSE_vol':>8} {'Bias_vol':>8} "
          f"{'ESS':>8} {'VarRatio':>8} {'EKF_gap':>8} {'Time':>6}")
    print("-" * 105)
    ...
    for mode in modes:
        ...
        s = {
            ...
            "ekf_gap": avg("mean_ekf_gap"), "time": avg("time_sec"),
        }
        summary[mode] = s
        print(f"{mode:<15} {s['rmse_h']:>8.4f} {s['bias_h']:>+8.4f} {s['rmse_vol']:>8.4f} "
              f"{s['bias_vol']:>+8.4f} {s['ess']:>8.1f} {s['var_ratio']:>8.3f} "
              f"{s['ekf_gap']:>+8.3f} {s['time']:>5.1f}s")

    print("\n--- Deltas vs base ---")
    base = summary["base"]
    for mode in ["ekf_gap", "ekf_feedback", "iw"]:
        m = summary[mode]
        ...
        tag_v = "better" if dv > 0 else "worse"
        print(f"  {mode:>14}: RMSE_h {dh:+.1f}% ({tag_h})  |Bias_h| {db:+.4f} ({tag_b})  RMSE_vol {dv:+.1f}% ({tag_v})")
