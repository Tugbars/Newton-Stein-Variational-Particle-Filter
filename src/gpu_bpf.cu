/**
 * @file gpu_bpf.cu
 * @brief GPU Bootstrap SIR Particle Filter
 *
 * State:       h_t = mu + rho*(h_{t-1} - mu) + sigma_z * eps_t
 * Observation: y_t = exp(h_t / 2) * eta_t
 *
 * Kernels: propagate+weight (fused), exp-subtract, scale+wh, resample
 * Reductions: thrust max, reduce, inclusive_scan
 */
...
// =============================================================================
// Simple host-side PCG32 for resampling uniform
// =============================================================================
...
float gpu_bpf_step(GpuBpfState* s, float y_t) {
    int n = s->n_particles;
...
double gpu_bpf_run_rmse(
    ...
    for (int t = 0; t < n_ticks; t++) {
        float h_est = gpu_bpf_step(state, (float)returns[t]);
        
        if (t >= skip) {
            double err = (double)h_est - true_h[t];
            sum_sq += err * err;
            count++;
        }
    }
